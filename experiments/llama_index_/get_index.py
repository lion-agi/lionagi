class BaseIndex:
    ...
    @staticmethod
    def _get_index(input_ = None, 
                
        # default to OpenAI
        llm = None, llm_provider = None, llm_kwargs = {},
        service_context = None, service_context_kwargs = {},
        index_type = None, index_kwargs = {},
        rerank_ = False, reranker_type = None, reranker = None, 
        rerank_kwargs = {}, get_engine = False, engine_kwargs = {},
        from_storage=False, storage_context = None, strorage_context_kwargs = {},
        index_id=None, load_index_from_storage_kwargs = {}
    ):
        """
        ...
        if from_storage:
            ...
            if rerank_:
                if not reranker:
                    ...
                    reranker = reranker_type(service_context=service_context, **rerank_kwargs)
        ...
        if not service_context:
            ...
            service_context = ServiceContext.from_defaults(llm=llm, **service_context_kwargs)
...
class LlamaIndex:
    ...
    def kg_index(input_ = None, 
        ...
        get_engine = False, engine_kwargs = {"similarity_top_k": 3, "response_mode": "tree_summarize"}, 
        ...
        if not from_storage:
            ...
            if storage_context is None:
                storage_context = StorageContext.from_defaults(graph_store=graph_store, **strorage_context_kwargs)
        ...
        if storage_context is None:
            from llama_index.graph_stores import SimpleGraphStore
            from llama_index.storage.storage_context import StorageContext
            storage_context = StorageContext.from_defaults(graph_store=SimpleGraphStore(), **strorage_context_kwargs)
    

        return BaseIndex._get_index(input_=input_, 
            llm = llm, llm_provider = llm_provider, llm_kwargs = llm_kwargs,
            service_context = service_context, service_context_kwargs = service_context_kwargs,
            index_type = index_type_, index_kwargs = index_kwargs,
            rerank_ = rerank_, reranker_type = reranker_type, reranker = reranker, 
            rerank_kwargs = rerank_kwargs, get_engine = get_engine, engine_kwargs = engine_kwargs,
            from_storage=from_storage, storage_context = storage_context, strorage_context_kwargs = strorage_context_kwargs,
            index_id=index_id, load_index_from_storage_kwargs=load_index_from_storage_kwargs
        )

    @staticmethod
    def vector_index(input_ = None, 
        # default to OpenAI
        llm = None, llm_provider = None, llm_kwargs = {
            "temperature": 0.1, "model": "gpt-4-1106-preview"},
