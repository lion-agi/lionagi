...
class TestIsEncrypted(unittest.TestCase):
    ...
    def test_with_encrypted_file(self):
        self.assertTrue(
            EncrytionUtil.is_encrypted(self.temp_file.name + '.enc', self.valid_key))

    def test_with_non_encrypted_file(self):
        self.assertFalse(
            EncrytionUtil.is_encrypted(self.non_encrypted_file.name, self.valid_key))
...
class TestDecodeBase64(unittest.TestCase):
    ...
    def test_with_invalid_base64_string(self):
        ...
        with self.assertRaises(
                Exception):  # Replace Exception with the specific exception if known
