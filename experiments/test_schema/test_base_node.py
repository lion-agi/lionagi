from lionagi.core.generic.base.base_node import *
import unittest
...
class TestBaseNode(unittest.TestCase):
    def setUp(self):
        ...
        self.edge1 = Edge(
            source_node_id=self.node1.id_, target_node_id=self.node2.id_, label="Edge 1"
        )
        self.edge2 = Edge(
            source_node_id=self.node2.id_, target_node_id=self.node3.id_, label="Edge 2"
        )
    ...
    def test_relations_property(self):
        ...
        self.assertEqual(
            self.node2.relations,
            {self.node1.id_: self.edge1, self.node3.id_: self.edge2},
        )
    ...
    def test_add_edge_method(self):
        ...
        self.assertTrue(
            self.node1.add_edge(node4, label="Edge 3", direction=EdgeDirection.OUT)
        )
        ...
        edge = Edge(
            source_node_id=self.node1.id_, target_node_id=node4.id_, label="Edge 4"
        )
