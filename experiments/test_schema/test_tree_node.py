...
from lionagi.new.schema.tree_node import TreeNode, TreeLabel, Edge
...
class TestTreeNode(unittest.TestCase):
    ...
    def test_add_parent_success(self, mock_add_edge):
        ...
        mock_add_edge.assert_called_once_with(self.parent_node, label=TreeLabel.CHILD)
        self.assertEqual(self.node.parent, self.parent_node)
    ...
    def test_add_child_success(self, mock_add_edge):
        ...
        mock_add_edge.assert_called_once_with(self.child_node1, label=TreeLabel.PARENT)
    ...
    def test_has_child_true(self):
        self.node.out_relations = {
            self.child_node1.id_: MagicMock(spec=Edge, label=TreeLabel.PARENT)
        }
    ...
    def test_has_child_false_wrong_label(self):
        self.node.out_relations = {
            self.child_node1.id_: MagicMock(spec=Edge, label="other_label")
        }
        result = self.node.has_child(self.child_node1)
