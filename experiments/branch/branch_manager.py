...
class RequestTitle(str, Enum):
    """
    An enumeration of valid package titles for communication requests.

    Attributes:
        MESSAGES (str): Represents a package carrying messages.
        TOOL (str): Represents a package for tools information or tools.
        SERVICE (str): Represents a package related to services.
        LLMCONFIG (str): Represents a package to configure or modify LLM settings.
    """
    MESSAGES = 'messages'
    TOOL = 'tools'
    SERVICE = 'provider'
...
class Request:
    """
    Represents a package for communication between components in the system.

    Args:
        from_name (str): The name of the sender.
        to_name (str): The name of the recipient.
        title (Union[str, RequestTitle]): The category of the package, indicating its type or category.
        request (Any): The actual content or data of the package.

    Raises:
        ValueError: If the package category is invalid or not recognized.
    """
    ...
    def __init__(self, from_name, to_name, title, request):
        ...
        try:
            ...
            if isinstance(title, RequestTitle):
                ...
                raise ValueError(
                    f'Invalid package category. Valid titles are {list(RequestTitle)}')
        except:
            raise ValueError(
                f'Invalid package category. Valid titles are {list(RequestTitle)}')
