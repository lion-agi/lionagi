...
class IOUtil:
    ...
    def to_csv(input: List[Dict[str, Any]],
        ...
        if not input:
            return
        if not os.path.exists(os.path.dirname(filepath)) and os.path.dirname(
                filepath) != '':
            if file_exist_ok:
    ...
    def to_temp(input: Any) -> tempfile.NamedTemporaryFile:
        """
        Writes the given input_ to a temporary file in JSON format.

        Args:
            input: The input_ data to write to the file. Can be a string or an iterable.

        Returns:
            A NamedTemporaryFile object representing the temporary file.

        Raises:
            TypeError: If the input_ data is not JSON serializable.
