...
from lionagi.experiments.io.io_util import IOUtil
...
class TestIOUtil(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_output = [{'name': 'Alice', 'age': '30'},
                                {'name': 'Bob', 'age': '25'}]
        self.valid_jsonl_data = '{"name": "Alice", "age": 30}\n{"name": "Bob", "age": 25}\n'
        self.expected_valid_output = [{'name': 'Alice', 'age': 30},
                                      {'name': 'Bob', 'age': 25}]
    ...
    def test_read_csv_varied_data_types(self, mock_file):
        expected_output = [{'name': 'Alice', 'age': '30', 'score': '85.5'},
    ...
    def test_write_json_empty_list(self):
        with patch("builtins.open", mock_open()) as mocked_file:
            ...
            mocked_file().write.assert_called_once_with(
                json.dumps(self.empty_data, indent=4))

    def test_write_json_non_serializable_data(self):
        non_serializable_data = [{'name': 'Alice', 'age': 30},
                                 {'name': 'Bob', 'age': lambda x: x}]
    ...
    def test_to_csv_nonexistent_dir_file_exist_ok_false(self, mock_exists):
        with self.assertRaises(FileNotFoundError):
            IOUtil.to_csv(self.valid_data, "nonexistent_dir/test.csv",
                          file_exist_ok=False)
    ...
    def test_to_csv_nonexistent_dir_file_exist_ok_true(self, mock_exists, mock_makedirs):
        ...
        with patch("builtins.open", mock_file):
            ...
            mock_makedirs.assert_called_once_with(
                os.path.dirname("nonexistent_dir/test.csv"), exist_ok=True)
