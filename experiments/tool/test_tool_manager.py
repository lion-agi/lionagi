from old_core.session.tool_manager import *
...
class TestToolManager(unittest.TestCase):
    def setUp(self):
        ...
        self.tool = Tool(func=lambda x: x, schema_={"function": {"name": "test_func"}})
...
class TestToolInvocation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.async_tool = Tool(
            func=async_tool_func, schema_={"function": {"name": "async_test_func"}}
        )
        self.sync_tool = Tool(
...
class TestFunctionCallExtraction(unittest.TestCase):
    ...
    def test_get_function_call_valid(self):
        ...
        response = {"action": "action_test_func", "arguments": json.dumps({"x": 10})}
        func_call = self.manager.get_oai_function_call(response)
    ...
    def test_get_function_call_invalid(self):
        ...
        with self.assertRaises(ValueError):
            self.manager.get_oai_function_call({})
