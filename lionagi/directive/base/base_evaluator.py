...
class BaseEvaluator:
    ...
    Examples:
        ...
        # Evaluating with unary operations
        print(evaluator.evaluate("-5", {}))  # Output: -5
        print(evaluator.evaluate("not (x > 5)", {"x": 3}))  # Output: True
    """
    ...
    def _evaluate_node(self, node: ast.AST, context: Dict[str, Any]) -> Any:
        ...
        node_key = (str(node), tuple(sorted(context.items())))
        if node_key in self.cache:
    ...
    def _validate_node(self, node):
        if isinstance(node, (ast.BinOp, ast.Compare, ast.BoolOp, ast.Name, ast.Constant)):
            if isinstance(node, ast.BinOp) and type(
                    node.op) not in self.allowed_operators:
