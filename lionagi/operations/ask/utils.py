...
class AskAnalysis(BaseModel):
    ...
    BRANCH_PROMPT: ClassVar[str] = """Query for branch {target}:
{query}"""

    MODEL_PROMPT: ClassVar[str] = """Query for model:
{query}"""

    EXTERNAL_PROMPT: ClassVar[str] = """Query for external system {target}:
{query}"""
    ...
    @classmethod
    def format_prompt(cls, query: str, target: Union["Branch", iModel, str]) -> str:
