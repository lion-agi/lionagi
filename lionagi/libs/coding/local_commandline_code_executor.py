...
from .base import CodeBlock, CodeExtractor, CommandLineCodeResult
...
$functions"""
    ...
    def sanitize_command(lang: str, code: str) -> None:
        ...
        dangerous_patterns = [
            ...
            (
                r"\bmv\b.*?\s+/dev/null",
                "Moving files to /dev/null is not allowed.",
            ),
            (r"\bdd\b", "Use of 'dd' command is not allowed."),
            (
                r">\s*/dev/sd[a-z][1-9]?",
                "Overwriting disk blocks directly is not allowed.",
            ),
        ...
        if lang in ["bash", "shell", "sh"]:
            for pattern, message in dangerous_patterns:
                if re.search(pattern, code):
                    raise ValueError(
                        f"Potentially dangerous command detected: {message}"
                    )
    ...
    def _setup_functions(self) -> None:
        ...
        if len(required_packages) > 0:
            ...
            if result.returncode != 0:
                raise ValueError(
                    f"Pip install failed. {result.stdout}, {result.stderr}"
                )
    ...
    def _execute_code_dont_check_setup(
        self, code_blocks: list[CodeBlock]
    ) -> CommandLineCodeResult:
        logs_all = ""
        file_names = []
