...
class CodeExecutorFactory:
    ...
    def create(code_execution_config: CodeExecutionConfig) -> CodeExecutor:
        ...
        if executor == "ipython-embedded":
            from .jupyter.embedded_ipython_code_executor import (
                EmbeddedIPythonCodeExecutor,
            )

            return EmbeddedIPythonCodeExecutor(
                **code_execution_config.get("ipython-embedded", {})
            )
        elif executor == "commandline-local":
            from .local_commandline_code_executor import (
                LocalCommandLineCodeExecutor,
            )

            return LocalCommandLineCodeExecutor(
                **code_execution_config.get("commandline-local", {})
            )
        else:
