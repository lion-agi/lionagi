...
class MongoDBAtlasVectorDB(VectorDB):
    ...
    def __init__(
        ...
        if collection_name:
            self.active_collection = self.create_collection(
                collection_name, overwrite
            )
        else:
    ...
    def _wait_for_index(
        ...
        assert action in [
            "create",
            "delete",
        ], f"{action=} must be create or delete."
        start = monotonic()
        while monotonic() - start < self._wait_until_index_ready:
            if action == "create" and self._is_index_ready(
    ...
    def create_collection(
        ...
        if collection_name not in self.db.list_collection_names():
            ...
            self.create_index_if_not_exists(
                index_name=self.index_name, collection=coll
            )
            return coll
        ...
        for index in self.db[collection_name].list_search_indexes():
            ...
            if self._wait_until_index_ready:
                self._wait_for_index(
                    self.db[collection_name], index["name"], "delete"
                )
            ...
            if result_ids != input_ids:
                logger.warning(
                    ...
                    "result_ids not in input_ids: {out_diff}".format(
                        in_diff=input_ids.difference(result_ids),
                        out_diff=result_ids.difference(input_ids),
                    )
                )
        ...
        **kwargs,
        ...
        for query_text in queries:
            ...
            query_vector = np.array(
                self.embedding_function([query_text])
            ).tolist()[0]
