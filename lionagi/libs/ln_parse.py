...
class ParseUtil:
	...
	def extract_code_block(
			str_to_parse: str, language: str | None = None,
	...
	@staticmethod
	def md_to_json(
			str_to_parse: str, *, expected_keys: list[str] | None = None,
		...
		json_obj = ParseUtil.extract_code_block(
			str_to_parse, language="json",
			parser=parser or ParseUtil.fuzzy_parse_json
		)

		if expected_keys:
			if missing_keys := [key for key in expected_keys if
			                    key not in json_obj]:
				raise ValueError(
	...
	def _extract_docstring_details_google(func):
		...
		param_start_pos = next(
			(i + 1 for i in range(1, lines_len) if (
					lines[i].startswith("Args") or lines[i].startswith(
				"Arguments"
				) or lines[i].startswith("Parameters"))), 0, )
		current_param = None
		for i in range(param_start_pos, lines_len):
			if lines[i] == "":
				...
				if len(param_desc) == 1:
					params_description[
						current_param] += f" {param_desc[0].strip()}"
					continue
	...
	def _python_to_json_type(py_type):
		...
		type_mapping = {
			...
			"list": "array", "tuple": "array", "bool": "boolean",
			"dict": "object",
		}
	...
	def _func_to_schema(func, style="google"):
		...
		for name, param in sig.parameters.items():
			...
			parameters["properties"][name] = {
				"type": param_type, "description": param_description,
			}

		return {
			"type": "function", "function": {
				"name": func_name, "description": func_description,
...
class StringMatch:
	...
	def choose_most_similar(word, correct_words_list, score_func=None):
		...
		# Calculate Jaro-Winkler similarity scores for each potential match
		scores = np.array(
			[score_func(convert.to_str(word), correct_word) for correct_word
				in correct_words_list]
		)
		# Find the index of the highest score
		max_score_index = np.argmax(scores)
