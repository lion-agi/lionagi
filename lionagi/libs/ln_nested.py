...
def nget(
		nested_structure: dict | list, indices: list[int | str],
	...
	try:
		...
		if (isinstance(target_container, list) and isinstance(
			last_index, int
...
# nested merge
def nmerge(
		nested_structure: list[dict | list], /, *, overwrite: bool = False,
		dict_sequence: bool = False, sequence_separator: str = "_",
		sort_list: bool = False,
		custom_sort: Callable[[Any], Any] | None = None, ) -> dict | list:
	"""
...
# flatten dictionary
def flatten(
		nested_structure: Any, /, *, parent_key: str = "", sep: str = "_",
		max_depth: int | None = None, inplace: bool = False,
		dict_only: bool = False, ) -> dict | None:
	"""
	...
	if inplace:
		...
		_dynamic_flatten_in_place(
			nested_structure, parent_key=parent_key, sep=sep,
			max_depth=max_depth, dict_only=dict_only, )
	else:
		parent_key_tuple = tuple(parent_key.split(sep)) if parent_key else ()
		return convert.to_dict(
			_dynamic_flatten_generator(
				nested_structure, parent_key=parent_key_tuple, sep=sep,
				max_depth=max_depth, dict_only=dict_only, )
		)
...
def unflatten(
		...
		max_depth: int | None = None, ) -> dict | list:
	"""
	...
	for composite_key, value in flat_dict.items():
		...
		ninsert(
			unflattened, indices=parts, value=value, sep=sep,
			max_depth=max_depth
			)
...
def ninsert(
		nested_structure: dict | list, /, indices: list[str | int],
		value: Any, *, sep: str = "_", max_depth: int | None = None,
	...
	for i, part in enumerate(indices[:-1]):
		...
		if isinstance(part, int):
			...
			if nested_structure[part] is None or not isinstance(
				...
				nested_structure[part] = [] if isinstance(
					next_part, int
					) else {}
...
def get_flattened_keys(
		nested_structure: Any, /, *, sep: str = "_",
		max_depth: int | None = None, dict_only: bool = False,
		inplace: bool = False, ) -> list[str]:
	"""
	...
	if not inplace:
		return convert.to_list(
			flatten(
				nested_structure, sep=sep, max_depth=max_depth,
				dict_only=dict_only
			).keys()
		)
...
def _dynamic_flatten_in_place(
		nested_structure: Any, /, *, parent_key: str = "", sep: str = "_",
		max_depth: int | None = None, current_depth: int = 0,
		dict_only: bool = False, ) -> None:
	"""
	...
	if isinstance(nested_structure, dict):
		...
		for k, v in items:
			...
			if isinstance(v, dict) and (
				...
				_dynamic_flatten_in_place(
					v, parent_key=new_key, sep=sep, max_depth=max_depth,
					current_depth=(current_depth + 1), dict_only=dict_only, )
...
def _handle_list_insert(
		nested_structure: list, part: int, value: Any
		) -> None:
	"""
...
def _dynamic_flatten_generator(
		nested_structure: Any, parent_key: tuple[str, ...], sep: str = "_",
		max_depth: int | None = None, current_depth: int = 0,
...
def _merge_dicts(
		iterables: list[dict[Any, Any]], dict_update: bool,
		dict_sequence: bool, sequence_separator: str, ) -> dict[Any, Any]:
	"""
...
def _merge_sequences(
		iterables: list, sort_list: bool,
		custom_sort: Callable[[Any], Any] | None = None, ) -> list[Any]:
	"""
...
def _filter_dict(
		...
		condition: Callable[[tuple[Any, Any]], bool]
) -> dict[Any, Any]:
	"""
...
def _filter_list(lst: list[Any], condition: Callable[[Any], bool]) -> list[
	Any]:
	"""
