...
def lcall(
		input_: Any, /, func: Callable, *, flatten: bool = False,
...
async def alcall(
		input_: Any | None = None, func: Callable = None, *,
		flatten: bool = False, dropna=False, **kwargs, ) -> list[Any]:
...
async def mcall(
	...
	if explode:
		...
		tasks = [AsyncUtil.handle_async_sync(func, inp, **kwargs) for
			inp, func in zip(inputs_, funcs_)]
	else:
		raise ValueError(
...
async def tcall(
		func: Callable, *args, delay: float = 0, err_msg: str | None = None,
	...
	return await async_call() if AsyncUtil.is_coroutine_func(
		func
		) else sync_call()
...
async def _tcall(
		func: Callable, *args, delay: float = 0, err_msg: str | None = None,
		ignore_err: bool = False, timing: bool = False, default: Any = None,
		timeout: float | None = None, **kwargs, ) -> Any:
	"""
	...
	try:
		...
		print(err_msg)
		if ignore_err:
			return ((default, SysUtil.get_now(
				datetime_=False
				) - start_time) if timing else default)
		else:
		...
		def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
			if AsyncUtil.is_coroutine_func(func):
				...
				async def async_wrapper(*args, **kwargs) -> Any:
					...
					preprocessed_kwargs = {k: preprocess(v) for k, v in
					                       kwargs.items()}
					result = await func(
...
class Throttle:
	...
	async def __call_async__(self, func: Callable[..., Any]) -> Callable[
		..., Any]:
		"""
