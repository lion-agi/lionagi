...
class SysUtil:
	...
	def is_schema(dict_: dict[Any, Any], schema: dict[Any, type]) -> bool:
		...
		return all(
			isinstance(dict_.get(key), expected_type) for key, expected_type
			in schema.items()
		)
	...
	def create_copy(input_: Any, num: int = 1) -> Any | list[Any]:
		...
		return (
			copy.deepcopy(input_) if num == 1 else [copy.deepcopy(input_) for
			                                        _ in range(num)])
	...
	@staticmethod
	def get_bins(input_: list[str], upper: int | None = 2000) -> list[
		list[int]]:
	...
	@staticmethod
	def install_import(
			package_name: str, module_name: str = None,
			import_name: str = None, pip_name: str = None, ) -> None:
	...
	@staticmethod
	def check_import(
			package_name: str, module_name: str | None = None,
			import_name: str | None = None, pip_name: str | None = None,
			attempt_install: bool = True, error_message: str = "", ) -> None:
	...
	def list_installed_packages() -> list:
		...
		return [dist.metadata["Name"] for dist in
	...
	@staticmethod
	def clear_dir(
			dir_path: Path | str, recursive: bool = False,
	...
	@staticmethod
	def create_path(
			directory: Path | str, filename: str, timestamp: bool = True,
