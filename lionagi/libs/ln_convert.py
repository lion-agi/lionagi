...
@singledispatch
def to_list(input_, /, *, flatten: bool = True, dropna: bool = True) -> list[
	Any]:
	"""
	...
	try:
		...
		return _flatten_list(
			iterable_list, dropna
			) if flatten else iterable_list
...
def _(
		input_, /, *args, orient: str = "list", as_list: bool = False,
		**kwargs
...
@singledispatch
def to_df(
		input_: Any, /, *, how: str = "all",
		drop_kwargs: dict[str, Any] | None = None, reset_index: bool = True,
...
@to_df.register(list)
def _(
		input_, /, *, how: str = "all", drop_kwargs: dict | None = None,
		reset_index: bool = True, **kwargs, ) -> pd.DataFrame:
	if not isinstance(
...
def to_num(
		input_: Any, /, *, upper_bound: int | float | None = None,
...
def to_readable_dict(input_: Any | list[Any]) -> str | list[Any]:
	...
	try:
		...
		return json.dumps(dict_, indent=4) if isinstance(
			input_, dict
			) else input_
	except Exception as e:
		raise ValueError(
...
def _str_to_num(
		input_: str, upper_bound: float | None = None,
		lower_bound: float | None = None, num_type: Type[int | float] = int,
		precision: int | None = None, ) -> int | float:
...
def _flatten_list(lst_: list[Any], dropna: bool = True) -> list[Any]:
	...
	return list(
		_dropna_iterator(flattened_list)
		) if dropna else flattened_list
