...
class APIUtil:
	...
	async def unified_api_call(
			http_session: aiohttp.ClientSession, method: str, url: str,
			**kwargs
	) -> Any:
		"""
		...
		for attempt in range(retry_count):
			async with api_call(url, **kwargs) as response:
				...
				if (APIUtil.api_rate_limit_error(
					response_json
	...
	async def retry_api_call(
			http_session: aiohttp.ClientSession, url: str, retries: int = 3,
			backoff_factor: float = 0.5, **kwargs, ) -> Any:
		"""
	...
	async def upload_file_with_retry(
			http_session: aiohttp.ClientSession, url: str, file_path: str,
			param_name: str = "file",
	...
	async def get_oauth_token_with_cache(
			http_session: aiohttp.ClientSession, auth_url: str,
			client_id: str, client_secret: str, scope: str, ) -> str:
		"""
	...
	def calculate_num_token(
			payload: Mapping[str, Any] = None, api_endpoint: str = None,
			token_encoding_name: str = None, ) -> int:  # sourcery skip: avoid-builtin-shadow
		"""
	...
	def create_payload(
		...
		for key in optional_:
			if bool(config[key]) and convert.strip_lower(
					config[key]
					) != "none":
				payload[key] = config[key]
...
class BaseRateLimiter(ABC):
	...
	def __init__(
			self, max_requests: int, max_tokens: int, interval: int = 60,
			token_encoding_name=None, ) -> None:
	...
	async def request_permission(self, required_tokens) -> bool:
		...
		async with self._lock:
			if (
					self.available_request_capacity > 0 and self.available_token_capacity > 0):
	...
	async def _call_api(
			self, http_session, endpoint: str, base_url: str, api_key: str,
			max_attempts: int = 3, method: str = "post",
			payload: Mapping[str, any] = None, **kwargs, ) -> Mapping[
				                                                  str, any] | None:
		"""
		...
		while True:
			if (
					self.available_request_capacity < 1 or self.available_token_capacity < 10):  # Minimum token count
			...
			if await self.request_permission(required_tokens):
				...
				while attempts_left > 0:
					try:
						...
						logging.warning(
							f"API call failed with exception: {e}"
							)
						attempts_left -= 1
	...
	@classmethod
	async def create(
			cls, max_requests: int, max_tokens: int, interval: int = 60,
			token_encoding_name=None, ) -> "BaseRateLimiter":
		"""
...
class EndPoint:
	...
	def __init__(
			self, max_requests: int = 1000, max_tokens: int = 100000,
			interval: int = 60, endpoint_: str | None = None,
			rate_limiter_class: Type[BaseRateLimiter] = SimpleRateLimiter,
			token_encoding_name=None, config: Mapping = None, ) -> None:
...
class BaseService:
	...
	def __init__(
			...
			schema: Mapping[str, Any] = None, token_encoding_name: str = None,
			max_tokens: int = 100_000, max_requests: int = 1_000,
	...
	async def init_endpoint(
			self,
			endpoint_: Sequence | str | EndPoint | None = None, ) -> None:
		"""
			...
			for ep in endpoint_:
				...
				if ep not in self.endpoints:
					...
					if isinstance(ep, EndPoint):
						...
						self.endpoints[ep] = EndPoint(
							max_requests=(endpoint_config.get(
								"max_requests", 1000
								) if endpoint_config.get(
								"max_requests", 1000
								) is not None else 1000), max_tokens=(
								endpoint_config.get(
									"max_tokens", 100000
									) if endpoint_config.get(
									"max_tokens", 100000
									) is not None else 100000), interval=(
								endpoint_config.get(
									"interval", 60
									) if endpoint_config.get(
									"interval", 60
									) is not None else 60), endpoint_=ep,
							token_encoding_name=self.token_encoding_name,
							config=endpoint_config, )
	...
	async def call_api(self, payload, endpoint, method, **kwargs):
		...
		async with aiohttp.ClientSession() as http_session:
			return await self.endpoints[endpoint].rate_limiter._call_api(
				http_session=http_session, endpoint=endpoint,
				base_url=self.base_url, api_key=self.api_key, method=method,
...
class PayloadPackage:
	...
	def chat_completion(cls, messages, llmconfig, schema, **kwargs):
		...
		return APIUtil.create_payload(
			input_=messages, config=llmconfig, required_=schema["required"],
			optional_=schema["optional"], input_key="messages", **kwargs, )
	...
	def fine_tuning(cls, training_file, llmconfig, schema, **kwargs):
		...
		return APIUtil._create_payload(
			input_=training_file, config=llmconfig,
			required_=schema["required"], optional_=schema["optional"],
			input_key="training_file", **kwargs, )
