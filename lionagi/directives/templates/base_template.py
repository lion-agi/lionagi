...
class BaseManualTemplate:
    ...
    def _render_conditionals(self, context: Dict[str, Union[str, int, float]]) -> str:
        ...
        return conditional_pattern.sub(lambda match: self._evaluate_condition(match, context), self.template_str)
    ...
    def _render_placeholders(self, rendered_template: str, context: Dict[str, Union[str, int, float, Callable]]) -> str:
        return re.sub(r'\{(\w+)\}', lambda match: self._replace_callable(match, context), rendered_template)
