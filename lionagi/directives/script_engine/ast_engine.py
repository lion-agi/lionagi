    ...
    def _evaluate_node(self, node, context):
        if isinstance(node, ast.Compare):
            ...
            for operation, comparator in zip(node.ops, node.comparators):
                ...
                if not op_func:
                    raise ValueError(f"Operation {type(operation).__name__} is not allowed.")
            ...
            raise ValueError("Unsupported AST node type encountered in condition evaluation.")
    ...
    def execute(self, script):
        ...
        for stmt in tree.body:
            if isinstance(stmt, ast.Assign):
                var_name = stmt.targets[0].id  # Assumes single target assignment for simplicity
                ...
                raise ValueError("Unsupported statement type encountered in script execution.")
