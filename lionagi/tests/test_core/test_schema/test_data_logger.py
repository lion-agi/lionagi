...
class TestDLog(unittest.TestCase):
    def test_serialize_default(self):
        log = DLog(input_data={"key1": "value1", "key2": {"nested": "value"}}, output_data="output")
        serialized = log.serialize()
        self.assertEqual(serialized["input_data"], '{"key1": "value1", "key2[^_^]nested": "value"}')
    ...
    def test_serialize_flatten_false(self):
        ...
        self.assertEqual(serialized["input_data"], '{"key1": "value1", "key2": {"nested": "value"}}')
        self.assertEqual(serialized["output_data"], "output")
        self.assertIsInstance(serialized["timestamp"], str)
    ...
    def test_deserialize_default(self):
        ...
        self.assertEqual(log.input_data, {"key1": "value1", "key2": {"nested": "value"}})
        self.assertEqual(log.output_data, "output")

    def test_deserialize_unflatten_false(self):
        input_str = '{"key1": "value1", "key2[^_^]nested": "value"}'
        output_str = "output"
        log = DLog.deserialize(input_str=input_str, output_str=output_str, unflatten_=False)
        self.assertEqual(log.input_data, '{"key1": "value1", "key2[^_^]nested": "value"}')
        self.assertEqual(log.output_data, "output")
