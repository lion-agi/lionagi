...
class TestForm(unittest.TestCase):
    ...
    def test_initial_fields(self):
        ...
        self.assertEqual(self.form.work_fields, {'input1': None, 'input2': None, 'output': None})

    def test_fill_input_fields(self):
        self.form.fill(input1=1, input2=2)
        self.assertTrue(self.form.workable)
        self.assertFalse(self.form.filled)
        self.assertEqual(self.form.work_fields, {'input1': 1, 'input2': 2, 'output': None})
...
class TestReport(unittest.TestCase):
    ...
    def test_complex_assignment(self):
        ...
        self.assertEqual(self.report.work_fields, {'a': None, 'b': None, 'c': None, 'e': None, 'f': None, 'g': None, 'h': None})
        self.report.fill(c=5, e=6, f=7, g=8, h=10)
        self.assertEqual(self.report.work_fields, {'a': None, 'b': None, 'c': 5, 'e': 6, 'f': 7, 'g': 8, 'h': 10})
        self.assertFalse(self.report.filled)
        self.assertFalse(self.report.workable)
        with self.assertRaises(ValueError):
