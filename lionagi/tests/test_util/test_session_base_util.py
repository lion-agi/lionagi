...
class TestCreateMessage(unittest.TestCase):
    ...
    def test_create_instruction_message(self):
        ...
        message = MessageUtil.create_message(instruction=instruction_info, context=context)
    ...
    def test_create_response_message(self):
        ...
        self.assertEqual(message.content["response"], response_info['message']['content'])

    def test_error_on_multiple_roles(self):
        """Test error is raised when multiple roles are provided."""
        with self.assertRaises(ValueError):
            MessageUtil.create_message(system={"info": "info"}, instruction={"task": "task"})
...
class TestFilterMessagesBy(unittest.TestCase):
    ...
    def test_filter_by_time_range(self):
        ...
        filtered = MessageUtil.filter_messages_by(self.messages, start_time=start_time, end_time=end_time)
        self.assertEqual(len(filtered), 1)
...
class TestReadJson(unittest.TestCase):
    ...
    def test_read_json(self, mock_read_json):
        ...
        mock_df = pd.DataFrame({
            "node_id": ["1", "2"],
            "content": ["JSON Message 1", "JSON Message 2"]
        })
