...
class TestFuzzyParseJson(unittest.TestCase):
    ...
    def test_strict_mode(self):
        ...
        self.assertEqual(ParseUtil.fuzzy_parse_json(correct_json_str, strict=True), {"name": "John", "age": 30})
    ...
    def test_escape_chars_in_json(self):
        ...
        self.assertEqual(ParseUtil.escape_chars_in_json('Line 1\nLine 2'), 'Line 1\\nLine 2')
        self.assertEqual(ParseUtil.escape_chars_in_json('Quote: "'), 'Quote: \\"')

    def test_extract_code_block(self):
        """Test extracting and parsing code blocks from Markdown."""
        markdown = '```python\nprint("Hello, world!")\n```'
        result = ParseUtil.extract_code_block(markdown, language='python', parser=lambda x: x)
    ...
    def test_jaro_distance(self):
        ...
        self.assertAlmostEqual(StringMatch.jaro_distance("martha", "marhta"), 0.9444444444444445, places=7)
    ...
    def test_jaro_winkler_similarity(self):
        ...
        self.assertAlmostEqual(StringMatch.jaro_winkler_similarity("dixon", "dicksonx", scaling=0.1), 0.8133333333333332, places=7)
        self.assertAlmostEqual(StringMatch.jaro_winkler_similarity("martha", "marhta", scaling=0.1), 0.9611111111111111, places=7)
        self.assertAlmostEqual(StringMatch.jaro_winkler_similarity("", "", scaling=0.1), 1.0)
        self.assertAlmostEqual(StringMatch.jaro_winkler_similarity("abc", "abc", scaling=0.1), 1.0)
        self.assertAlmostEqual(StringMatch.jaro_winkler_similarity("abc", "def", scaling=0.1), 0.0)
