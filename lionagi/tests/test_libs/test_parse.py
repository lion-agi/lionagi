...
class TestFuzzyParseJson(unittest.TestCase):
	...
	def test_jaro_distance(self):
		...
		self.assertAlmostEqual(
			StringMatch.jaro_distance("martha", "marhta"), 0.9444444444444445,
			places=7
		)
	...
	def test_jaro_winkler_similarity(self):
		...
		self.assertAlmostEqual(
			StringMatch.jaro_winkler_similarity(
				...
				), 0.8133333333333332, places=7, )
		self.assertAlmostEqual(
			StringMatch.jaro_winkler_similarity(
				"martha", "marhta", scaling=0.1
				), 0.9611111111111111, places=7, )
		self.assertAlmostEqual(
			StringMatch.jaro_winkler_similarity("", "", scaling=0.1), 1.0
		)
		self.assertAlmostEqual(
			StringMatch.jaro_winkler_similarity("abc", "abc", scaling=0.1),
			1.0
		)
		self.assertAlmostEqual(
			StringMatch.jaro_winkler_similarity("abc", "def", scaling=0.1),
			0.0
		)
	...
	def test_func_to_schema(self, mock_extract):
		mock_extract.return_value = (
			"Function description", {"param1": "Description of param1."},)
		...
		self.assertIn(
			"param1", schema["function"]["parameters"]["properties"]
			)
		self.assertEqual(
