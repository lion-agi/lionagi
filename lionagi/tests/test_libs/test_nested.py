...
class TestGetFlattenedKeys(unittest.TestCase):
    ...
    def test_get_keys_with_max_depth(self):
        nested_dict = {'a': {'b': {'c': 1}}}
        expected_keys = ['a_b']
        self.assertEqual(set(get_flattened_keys(nested_dict, max_depth=1)), set(expected_keys))

    def test_get_keys_dict_only(self):
        nested_mix = {'a': [1, 2], 'b': {'c': 3}}
        expected_keys = ['a', 'b_c']
        self.assertEqual(set(get_flattened_keys(nested_mix, dict_only=True)), set(expected_keys))
