...
_message_fields = ["node_id", "timestamp", "role", "sender", "recipient",
                   "content"]
...
class BaseMessage(DataNode):
	...
	def __str__(self):
		content_preview = (
			f"{str(self.content)[:75]}..." if self.content and len(
				self.content
				) > 75 else str(self.content))
...
class Instruction(BaseMessage):
	...
	def __init__(
			self, instruction: dict | list | str, context=None,
			sender: str | None = None, output_fields=None,
			recipient=None, ):  # sourcery skip: avoid-builtin-shadow
		super().__init__(
			role="user", sender=sender or "user",
...
class System(BaseMessage):
	...
	def __init__(
			self, system: dict | list | str, sender: str | None = None,
			recipient=None
	):
		super().__init__(
			role="system", sender=sender or "system",
...
class Response(BaseMessage):
	...
	def __init__(
			self, response: dict | list | str, sender: str | None = None,
			recipient=None
	) -> None:
		content_key = ""
		try:
			...
			if convert.strip_lower(response["content"]) == "none":
				...
				try:
					...
					elif "action_request" in convert.to_dict(
							response["content"]
							):
						sender = sender or "action_request"
						content_key = content_key or "action_request"
		...
		try:
			...
			while tool_count < len(response["tool_calls"]):
				...
				if nested.nget(response, _path) == "function":
					...
					_path2 = ["tool_calls", tool_count, "function",
					          "arguments"]
