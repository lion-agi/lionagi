...
class MonoReAct(MonoChat):
	...
	async def ReAct(
			self, instruction: Instruction | str | dict[str, dict | str],
			context=None, sender=None, system=None, tools=None,
			num_rounds: int = 1, reason_prompt=None, action_prompt=None,
			output_prompt=None, **kwargs, ):
		return await self._ReAct(
			instruction, context=context, sender=sender, system=system,
			tools=tools, num_rounds=num_rounds, reason_prompt=reason_prompt,
			action_prompt=action_prompt, output_prompt=output_prompt,
			**kwargs, )

	@staticmethod
	def _get_prompt(
			prompt=None, default=None, num_steps=None, instruction=None
			):
	...
	async def _handle_auto(
			self, _out=None, out=None, instruction=None, output_prompt=None,
			sender=None, output=False, config=None, ):
	...
	async def _ReAct(
			self, instruction: Instruction | str | dict[str, dict | str],
			context=None, sender=None, system=None, tools=None,
			num_rounds: int = 1, auto=False, reason_prompt=None,
			action_prompt=None, output_prompt=None, out=True, **kwargs, ):
		...
		if auto:
			a = await self._handle_auto(
				instruction, output_prompt=output_prompt, sender=sender,
				out=out, output=True, config=config, )
			if a:
				return a

	async def _react(
			self, instruction=None, context=None, output_fields=None,
			tools=None, reason_prompt=None, action_prompt=None, **kwargs, ):

		config = self._create_followup_config(
			tools=tools, auto=False, **kwargs
			)
		...
		output_fields = {
			...
			"action": action_prompt or "the action(s) to take, in function call format. If no actions are needed return none",
		}
		...
		if convert.strip_lower(out_["action"]) not in ["none", "no", "na",
		                                               "nan"]:
			res = await self._invoke_tools(
				content_={"action_request": out_["action"]}
				)
