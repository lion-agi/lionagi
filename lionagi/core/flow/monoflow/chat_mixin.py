...
from lionagi.libs import (
	ln_nested as nested, ln_func_call as func_call, ln_convert as convert,
)
...
class MonoChatConfigMixin(ABC):
	...
	def _create_chat_config(
			self, instruction: Instruction | str | dict[str, Any],
			context: Any | None = None, sender: str | None = None,
			system: str | dict[str, Any] | None = None,
			tools: TOOL_TYPE = False, output_fields=None, **kwargs, ) -> Any:
		...
		if "tool_parsed" in kwargs:
			...
			kwargs = self.branch.tool_manager.parse_tool(
				tools=tools, **kwargs
				)
...
class MonoChatInvokeMixin(ABC):
	...
	def _return_response(content_, output_fields):
		...
		if len(content_.items()) == 1 and len(
				nested.get_flattened_keys(content_)
				) == 1:
		...
		if output_fields:
			try:
				...
				out_ = StringMatch.correct_keys(
					output_fields=output_fields, out_=out_
					)
			except Exception:
				pass
	...
	async def _invoke_tools(self, content_=None, func_calls_=None):
		...
		outs = await func_call.alcall(
			func_calls_, self.branch.tool_manager.invoke
			)
		...
		for out_, f in zip(outs, func_calls_):
			res = {
				"function": f[0], "arguments": f[1], "output": out_,
			}
	...
	def _process_chatcompletion(self, payload, completion, sender):
		if "choices" in completion:
			...
			self.branch.datalogger.append(
				input_data=payload, output_data=completion
				)
	...
	async def _call_chatcompletion(
			self, sender=None, with_sender=False, **kwargs
			):
		messages = (
			self.branch.chat_messages_with_sender if with_sender else self.branch.chat_messages)
