...
class MonoFollowup(MonoChat):
	...
	@staticmethod
	def _get_prompt(
			prompt=None, default=None, num_followup=None, instruction=None
			):
	...
	async def _handle_auto(
			...
			sender=None, output=False, **kwargs, ):
	...
	async def _handle_auto_output(
		...
		prompt = self._get_prompt(
			prompt=output_prompt, default=_output_prompt,
	...
	async def _followup(
			...
			context=None, sender=None, system=None, tools=None,
			max_followup: int = 1, auto=False, followup_prompt=None,
			output_prompt=None, out=True, **kwargs, ) -> None:
		_out = ""
		...
		while i < max_followup:
			...
			prompt_ = self._get_prompt(
				prompt=followup_prompt, default=_followup_prompt,
			...
			if i == 0:
				...
				_out = await self.chat(
					prompt_, context=context, system=system, sender=sender,
					**config
				)
