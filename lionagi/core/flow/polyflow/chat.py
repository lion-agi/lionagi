...
class PolyChat(BasePolyFlow):
	...
	async def _parallel_chat(
			self, instruction: Instruction | str, num_instances=1,
			context=None, sender=None, messages=None, tools=False, out=True,
			invoke: bool = True, output_fields=None, persist_path=None,
			branch_config={}, explode=False, include_mapping=True,
			default_key="response", **kwargs, ) -> Any:
		"""
		parallel chat
		"""
		...
		async def _inner(i, ins_, cxt_):
			...
			branch_ = Branch(
				...
				persist_path=persist_path, **branch_config, )
			...
			res_ = await branch_.chat(
				instruction=ins_ or instruction, context=cxt_ or context,
				sender=sender, tools=tools, invoke=invoke, out=out,
			...
			if include_mapping:
				return {
					...
					"context": cxt_ or context, "branch_id": branch_.id_,
					default_key: res_,
				}
		...
		async def _inner_3(i):
			...
			tasks = [_inner_2(i, ins_=ins_) for ins_ in
			         convert.to_list(instruction)]
			ress = await AsyncUtil.execute_tasks(*tasks)
			return convert.to_list(ress)

		async def _inner_3_b(i):
			"""different context but same instruction"""
			tasks = [_inner_2(i, cxt_=cxt_) for cxt_ in
			         convert.to_list(context)]
			ress = await AsyncUtil.execute_tasks(*tasks)
			return convert.to_list(ress)
		...
		async def _inner_4(i):
			...
			tasks = []
			if explode:
				tasks = [_inner_2(i, ins_=ins_, cxt_=cxt_) for ins_ in
					convert.to_list(instruction) for cxt_ in
					convert.to_list(context)]
			else:
				tasks = [_inner_2(i, ins_=ins_, cxt_=cxt_) for ins_, cxt_ in
					zip(
