...
class Session:
	...
	def __init__(
		...
		self.setup_default_branch(
			...
			tool_manager=tool_manager, service=service, llmconfig=llmconfig,
			tools=tools, persist_path=persist_path, datalogger=datalogger, )
	...
	@classmethod
	def from_csv(
			cls, filepath: PATH_TYPE,
			system: dict | list | System | None = None,
			sender: str | None = None,
			llmconfig: dict[str, str | int | dict] | None = None,
			service: BaseService = None, default_branch_name: str = "main",
			tools: TOOL_TYPE = False, # instruction_sets=None,
			tool_manager=None, **kwargs, ) -> "Session":
		"""
		...
		return cls(
			system=system, sender=sender, llmconfig=llmconfig,
			service=service, default_branch_name=default_branch_name,
			tools=tools, tool_manager=tool_manager, messages=df, **kwargs, )
	...
	def from_json(
		...
		return cls(
			...
			tool_manager=tool_manager, messages=df, **kwargs, )

	def to_csv_file(
			self, filename: str = "messages.csv", dir_exist_ok: bool = True,
		...
		for name, branch in self.branches.items():
			...
			branch.to_csv_file(
				filename=f_name, dir_exist_ok=dir_exist_ok,
				timestamp=timestamp, time_prefix=time_prefix, verbose=verbose,
				clear=clear, **kwargs, )

	def to_json_file(
			self, filename: str = "messages.json", dir_exist_ok: bool = False,
			timestamp: bool = True, time_prefix: bool = False,
			verbose: bool = True, clear: bool = True, **kwargs, ):
		"""
	...
	def log_to_csv(
			self, filename: str = "log.csv", dir_exist_ok: bool = True,
			timestamp: bool = True, time_prefix: bool = False,
			verbose: bool = True, clear: bool = True, **kwargs, ):
		"""
	...
	def log_to_json(
			self, filename: str = "log.json", dir_exist_ok: bool = True,
			timestamp: bool = True, time_prefix: bool = False,
			verbose: bool = True, clear: bool = True, **kwargs, ):
		"""
	...
	async def call_chatcompletion(
			self, branch: Branch | str | None = None,
			sender: str | None = None, with_sender=False, **kwargs, ):
		"""
		...
		await branch.call_chatcompletion(
			sender=sender, with_sender=with_sender, **kwargs, )

	async def chat(
			self, instruction: dict | list | Instruction | str,
			branch: Branch | str | None = None,
			context: dict | list | str = None, sender: str | None = None,
			system: dict | list | System | None = None,
			tools: TOOL_TYPE = False, out: bool = True, invoke: bool = True,
			output_fields=None, **kwargs, ) -> str | None:
		"""
		...
		return await branch.chat(
			...
			system=system, tools=tools, out=out, invoke=invoke,
			output_fields=output_fields, **kwargs, )

	async def ReAct(
			self, instruction: dict | list | Instruction | str,
			branch: Branch | str | None = None, context=None, sender=None,
			system=None, tools=None, auto=False, num_rounds: int = 1,
			reason_prompt=None, action_prompt=None, output_prompt=None,
			**kwargs, ):
		"""
		...
		return await branch.ReAct(
			instruction=instruction, context=context, sender=sender,
			system=system, tools=tools, auto=auto, num_rounds=num_rounds,
			reason_prompt=reason_prompt, action_prompt=action_prompt,
			output_prompt=output_prompt, **kwargs, )

	async def followup(
			self, instruction: dict | list | Instruction | str, branch=None,
			context=None, sender=None, system=None, tools=None,
			max_followup: int = 1, auto=False, followup_prompt=None,
		...
		return await branch.followup(
			instruction=instruction, context=context, sender=sender,
			system=system, tools=tools, max_followup=max_followup, auto=auto,
			followup_prompt=followup_prompt, output_prompt=output_prompt,
			out=out, **kwargs, )

	async def parallel_chat(
			self, instruction: Instruction | str, num_instances=1,
			context=None, sender=None, branch_system=None, messages=None,
			tools=False, out=True, invoke: bool = True, output_fields=None,
			persist_path=None, branch_config=None, explode=False,
			include_mapping=False, **kwargs, ):
		"""
		parallel chat
		"""
		...
		return await flow.parallel_chat(
			instruction, num_instances=num_instances, context=context,
			sender=sender, branch_system=branch_system, messages=messages,
			tools=tools, out=out, invoke=invoke, output_fields=output_fields,
			persist_path=persist_path, branch_config=branch_config,
			explode=explode, include_mapping=include_mapping, **kwargs, )
	...
	def new_branch(
			...
			system: dict | list | System | None = None,
			sender: str | None = None,
			messages: dataframe.ln_DataFrame | None = None, tool_manager=None,
			service=None, llmconfig=None, tools: TOOL_TYPE = False, ) -> None:
		...
		if isinstance(tools, Tool):
			tools = [tools]
		new_branch = Branch(
			name=branch_name, messages=messages, tool_manager=tool_manager,
	...
	def get_branch(
		...
		if isinstance(branch, str):
			...
			return ((self.branches[branch], branch) if get_name else
	...
	def delete_branch(
			self, branch: Branch | str, verbose: bool = True
			) -> bool:
	...
	def merge_branch(
			self, from_: str | Branch, to_branch: str | Branch,
	...
	def _verify_default_branch(self):
		if self.branches:
			if self.default_branch_name not in self.branches.keys():
				raise ValueError(
					"default branch name is not in imported branches"
					)
			if self.default_branch is not self.branches[
	...
	def _setup_default_branch(
			self, system, sender, default_branch, default_branch_name,
			messages, # instruction_sets,
			tool_manager, service, llmconfig, tools, persist_path,
			datalogger, ):
		...
		branch = default_branch or Branch(
			...
			# instruction_sets=instruction_sets,
			messages=messages, persist_path=persist_path,
