    ...
    def get_node_edges(
        ...
        if node_as in ['all', 'both']:
            ...
            if label:
                for k, v in all_node_edges.items():
                    all_node_edges[k] = (
                        v if v.label in convert.to_list(label, dropna=True, flatten=True) 
                        else []
                    )
            ...
            raise ValueError(f"Invalid value for self_as: {node_as}, must be 'head' or 'tail'")
    ...
    def relate_nodes(
        ...
        **kwargs,
        ...
        try:
            ...
            head.relate(tail, node_as="head", condition=condition, label=label, bundle=bundle,  **kwargs)
        except Exception as e:
            raise ValueError(f"Error adding edge: {e}") from e

    def remove_edge(self, edge: Edge | str | list[str | Edge] | dict[str, Edge]) -> bool:
        """
