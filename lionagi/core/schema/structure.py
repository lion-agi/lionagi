...
class Relationship(BaseRelatableNode):
	...
	def _is_in(self, obj: Dict[str, Any]) -> bool:
		...
		if self._source_existed(obj) and self._target_existed(obj):
			...
			raise ValueError(
				f"Target node {self.source_node_id} does not exist"
				)
		else:
			raise ValueError(
				f"Source node {self.target_node_id} does not exist"
				)
...
class Graph(BaseRelatableNode):
	...
	def get_node_relationships(
		...
		if out_edge:
			relationship_ids = list(
				self.node_relationships[node.id_]["out"].keys()
				)
			...
			relationship_ids = list(
				self.node_relationships[node.id_]["in"].keys()
				)
			relationships = func_call.lcall(
				relationship_ids, lambda i: self.relationships[i]
			)
	...
	def remove_relationship(self, relationship: Relationship) -> Relationship:
		...
		self.node_relationships[relationship.target_node_id]["in"].pop(
			relationship.id_
			)
	...
	def to_networkx(self, **kwargs) -> Any:
		...
		for _, relationship in self.relationships.items():
			...
			relationship_info.update(
				{"class_name": relationship.__class__.__name__}
				)
...
class Structure(BaseRelatableNode):
	...
	def add_relationship(
			self, from_node: BaseNode, to_node: BaseNode, bundle=False,
			condition=None, **kwargs, ):
		if isinstance(from_node, Tool) or isinstance(
				from_node, ActionSelection
				):
			raise ValueError(
		...
		relationship = Relationship(
			source_node_id=from_node.id_, target_node_id=to_node.id_,
	...
	async def check_condition(self, relationship, executable_id):
		if relationship.condition.source_type == "structure":
			...
			self.send(
				recipient_id=executable_id, category="condition",
	...
	async def get_next_step(self, current_node: BaseNode, executable_id):
		...
		for relationship in next_relationships:
			...
			if relationship.condition:
				check = await self.check_condition(
					relationship, executable_id
					)
				if not check:
					continue
	...
	def acyclic(self):
		...
		check_dict = {key: 0 for key in
			self.graph.nodes.keys()}  # 0: not visited, 1: temp, 2: perm
	...
	def send(self, recipient_id: str, category: str, package: Any) -> None:
		mail = BaseMail(
			sender_id=self.id_, recipient_id=recipient_id, category=category,
			package=package, )
	...
	def process_relationship_condition(self, relationship_id):
		for key in list(self.pending_ins.keys()):
			...
			while self.pending_ins[key]:
				...
				if (mail.category == "condition" and mail.package[
		...
		for key in list(self.pending_ins.keys()):
			while self.pending_ins[key]:
				...
				if not next_nodes:  # tail
					self.send(
						recipient_id=mail.sender_id, category="end",
						package="end"
					)
				else:
					if len(next_nodes) == 1:
						self.send(
							recipient_id=mail.sender_id, category="node",
							package=next_nodes[0], )
					else:
						self.send(
							recipient_id=mail.sender_id, category="node_list",
							package=next_nodes, )
