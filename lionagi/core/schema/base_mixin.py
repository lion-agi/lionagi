...
class BaseFromObjectMixin(ABC, BaseModel):
	...
	def _from_str(cls, obj: str, *args, fuzzy_parse=False, **kwargs) -> T:
		obj = ParseUtil.fuzzy_parse_json(
			obj
			) if fuzzy_parse else convert.to_dict(obj)
		try:
	...
	@classmethod
	def _from_pd_series(
			cls, obj: pd.Series, *args, pd_kwargs=None, **kwargs
			) -> T:
	...
	def _from_pd_dataframe(
		...
		if pd_kwargs is None:
			pd_kwargs = {}
		return [cls.from_obj(row, *args, **pd_kwargs, **kwargs) for _, row in
			obj.iterrows()]
...
class BaseMetaManageMixin(ABC, BaseModel):
	...
	def meta_has_key(
			self, key: str, flattened: bool = False, **kwargs
			) -> bool:
		"""
	...
	def meta_get(
			self, key: str, indices: list[str | int] = None,
			default: Any = None
	) -> Any:
		"""
	...
	def meta_merge(
		...
		"""
		nested.nmerge(
			[self.metadata, additional_metadata], overwrite=overwrite,
			**kwargs
		)
	...
	def meta_filter(self, condition: Callable[[Any, Any], bool]) -> dict[
		str, Any]:
		"""
...
class BaseComponentMixin(
	BaseFromObjectMixin, BaseToObjectMixin, BaseMetaManageMixin
	):
	pass
