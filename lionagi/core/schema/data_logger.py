...
class DLog:
	...
	def deserialize(
		...
		if unflatten_:
			...
			output_data = nested.unflatten(
				convert.to_dict(output_str), sep=sep
				)
...
class DataLogger:
	...
	def __init__(
			self, persist_path: str | Path | None = None,
		...
		"""
		self.persist_path = Path(persist_path) if persist_path else Path(
			"data/logs/"
			)
	...
	def to_csv_file(
		...
		try:
			logs = [log.serialize(flatten_=flatten_, sep=sep) for log in
			        self.log]
