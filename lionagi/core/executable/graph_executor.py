...
from lionagi.os.collections import Node, Edge, Graph, progression, Mail
from lionagi.os.core.executor.base_executor import BaseExecutor
from lionagi.os.core.action import Tool, DirectiveSelection, ActionNode
...
class GraphExecutor(BaseExecutor, Graph):
    ...
    async def _next_node(self, current_node: Node, executable_id, request_source):
        ...
        for edge in to_list(list(next_edges.values())):
            if edge.bundle:
                continue
            if edge.condition:
            ...
            bundled_nodes = deque()
            for f_edge in to_list(list(further_edges.values())):
                if f_edge.bundle:
    ...
    #     """
    #     from lionagi.integrations.storage.store.to_excel import to_excel
