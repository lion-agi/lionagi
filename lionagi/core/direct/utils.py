...
def _handle_single_out(
		out_, default_key, choices=None, to_type="dict", to_type_kwargs=None,
		to_default=True, ):
	...
	if (choices is not None and answer not in choices and convert.strip_lower(
		out_
		) in ["", "none", "null", "na", "n/a"]):
...
def _handle_multi_out(
		out_, default_key, choices=None, to_type="dict", to_type_kwargs=None,
		to_default=True, include_mapping=False, ):
	if to_type_kwargs is None:
		to_type_kwargs = {}
	if include_mapping:
		for i in out_:
			i[default_key] = _handle_single_out(
				i[default_key], choices=choices, default_key=default_key,
				to_type=to_type, to_type_kwargs=to_type_kwargs,
				to_default=to_default, )
	else:
		_out = []
		for i in out_:
			i = _handle_single_out(
				i, choices=choices, default_key=default_key, to_type=to_type,
				to_type_kwargs=to_type_kwargs, to_default=to_default, )
			_out.append(i)
