...
async def select(
		context, choices, *, num_choices=1, method="llm", objective=None,
		default_key="answer", reason=False, confidence_score=False,
		retry_kwargs=None, **kwargs, ):
	if retry_kwargs is None:
		retry_kwargs = {}
	return await _force_select(
		context=context, choices=choices, num_choices=num_choices,
		method=method, objective=objective, default_key=default_key,
		reason=reason, confidence_score=confidence_score,
...
async def _force_select(
	...
	async def _inner():
		...
		if isinstance(out_, dict) and out_[default_key] not in choices:
			v = StringMatch.choose_most_similar(
				out_.pop(default_key, ""), choices
				)
			out_[default_key] = v
...
def _create_select_config(
		choices, num_choices=1, objective=None, default_key="answer",
		reason=False, confidence_score=False, **kwargs, ):

	instruct = {
		"task": f"select {num_choices} from provided", "choices": choices
	}
...
async def _select(
	...
	_instruct, _output_fields, _kwargs = _create_select_config(
		choices=choices, num_choices=num_choices, objective=objective,
		default_key=default_key, reason=reason,
