...
async def predict(
		sentence, *, num_sentences=1, default_key="answer",
		confidence_score=False, reason=False, retry_kwargs=None,
		include_mapping=False, **kwargs, ):
	if retry_kwargs is None:
		retry_kwargs = {}
	return await _force_predict(
		sentence=sentence, num_sentences=num_sentences,
		default_key=default_key, confidence_score=confidence_score,
		reason=reason, retry_kwargs=retry_kwargs,
		include_mapping=include_mapping, **kwargs, )


async def _force_predict(
		sentence, num_sentences, default_key="answer", confidence_score=False,
		reason=False, retry_kwargs={}, include_mapping=False, **kwargs, ):
	...
	async def _inner2():
		out_ = await _parallel_predict(
			sentence=sentence, num_sentences=num_sentences,
			default_key=default_key, confidence_score=confidence_score,
			reason=reason, include_mapping=include_mapping, **kwargs, )
	...
	if (isinstance(sentence, (list, tuple)) and len(
			sentence
			) > 1) or include_mapping:
...
def _create_predict_config(
		num_sentences, default_key="answer", confidence_score=False,
		reason=False, **kwargs, ):
	instruct = {
...
async def _parallel_predict(
		sentence, num_sentences, default_key="answer", confidence_score=False,
		reason=False, include_mapping=False, **kwargs, ):
	_instruct, _output_fields, _kwargs = _create_predict_config(
		num_sentences=num_sentences, default_key=default_key,
		confidence_score=confidence_score, reason=reason, **kwargs, )
