...
async def score(
		context, instruction=None, *, score_range=(1, 10), inclusive=True,
		num_digit=0, default_key="score", method="llm", reason=False,
		confidence_score=False, retry_kwargs=None, **kwargs, ):
	if retry_kwargs is None:
		retry_kwargs = {}
	return await _force_score(
		context=context, instruction=instruction, score_range=score_range,
		inclusive=inclusive, num_digit=num_digit, default_key=default_key,
		method=method, reason=reason, confidence_score=confidence_score,
		retry_kwargs=retry_kwargs, **kwargs, )


async def _force_score(
		context, instruction=None, score_range=(1, 10), inclusive=True,
		num_digit=1, default_key="score", method="llm", reason=False,
		confidence_score=False, retry_kwargs={}, **kwargs, ):

	async def _inner():
		out_ = await _score(
			instruction=instruction, context=context, score_range=score_range,
			inclusive=inclusive, num_digit=num_digit, reason=reason,
			default_key=default_key, confidence_score=confidence_score,
...
def _create_score_config(
		instruction, score_range=(1, 10), inclusive=True, num_digit=0,
		reason=False, default_key="score", confidence_score=False,
		**kwargs, ):
	instruct = {
	...
	output_fields = {
		default_key: f"""a numeric score as {return_precision}"""
	}
...
async def _score(
		context, instruction=None, score_range=(1, 10), inclusive=True,
		num_digit=0, default_key="score", method="llm", reason=False,
		confidence_score=False, **kwargs, ):
	_instruct, _output_fields, _kwargs = _create_score_config(
		instruction=instruction, score_range=score_range, inclusive=inclusive,
		num_digit=num_digit, reason=reason, default_key=default_key,
	...
	if method == "llm":
		out_ = await branch.chat(
			_instruct, tools=None, context=context,
			output_fields=_output_fields, **_kwargs, )

	to_num_kwargs = {
		"upper_bound": score_range[1], "lower_bound": score_range[0],
