...
from lionagi.core.messages.schema import (
	System, Instruction, Response, BaseMessage, BranchColumns,
)
...
class MessageUtil:
	...
	def create_message(
			...
			response: Response | CUSTOM_TYPE = None, output_fields=None,
			**kwargs, ) -> BaseMessage:
		"""
		...
		if sum(func_call.lcall([system, instruction, response], bool)) != 1:
			raise ValueError(
				"Error: Message must have one and only one role."
				)
	...
	def validate_messages(messages: dataframe.ln_DataFrame) -> bool:
		...
		if any(
				role not in ["system", "user", "assistant"] for role in
	...
	def sign_message(
		...
		for i in df.index:
			if not df.loc[i, "content"].startswith("Sender"):
				df.loc[
					i, "content"] = f"Sender {sender}: {df.loc[i, 'content']}"
			else:
	...
	def filter_messages_by(
			messages: dataframe.ln_DataFrame, role: str | None = None,
			sender: str | None = None, start_time: datetime | None = None,
		...
		try:
			...
			if content_keywords:
				outs = MessageUtil.search_keywords(
					outs, keywords=content_keywords,
			...
			outs = outs[
				outs["timestamp"] > start_time] if start_time else outs
	...
	@staticmethod
	def remove_message(
			messages: dataframe.ln_DataFrame, node_id: str
			) -> bool:
		"""
		...
		messages.drop(
			messages[messages["node_id"] == node_id].index, inplace=True
			)
	...
	def get_message_rows(
			messages: dataframe.ln_DataFrame, sender: str | None = None,
			role: str | None = None, n: int = 1, sign_: bool = False,
			from_: str = "front", ) -> dataframe.ln_DataFrame:
		"""
	...
	def extend(
		...
		try:
			if len(df2.dropna(how="all")) > 0 and len(
					df1.dropna(how="all")
					) > 0:
		...
		for _, i in messages.iterrows():
			...
			if i.role == "assistant":
				with contextlib.suppress(Exception):
					...
					if a is not None:
						answers.extend(
							(f"Function: {a}", f"Arguments: {b}",
							 f"Output: {c}")
						)
					else:
						answers.append(
							nested.nget(content, ["assistant_response"])
							)
			elif i.role == "user":
				with contextlib.suppress(Exception):
