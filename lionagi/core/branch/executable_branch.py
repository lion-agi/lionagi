...
class ExecutableBranch(BaseRelatableNode):
	...
	async def _process_node(self, mail: BaseMail):
		...
		if isinstance(mail.package, System):
			...
			await self._instruction_process(
				mail.package, verbose=self.verbose
				)
			self.send(mail.sender_id, "node_id", mail.package.id_)
	...
	def _process_condition(self, mail: BaseMail):
		...
		back_mail = {
			"relationship_id": mail.package.id_, "check_result": check_result
		}
	...
	async def _action_process(self, action: ActionNode, verbose=True):
		...
		if self.context:
			result = await func(
				...
				context=self.context, tools=action.tools,
				**action.action_kwargs, )
			self.context = None
		else:
			result = await func(
				action.instruction.content, tools=action.tools,
				**action.action_kwargs
			)
