...
from ..messages.schema import (
	BranchColumns, System, Response, Instruction, BaseMessage,
)
...
class BaseBranch(BaseRelatableNode, ABC):
	...
	def __init__(
			self, messages: dataframe.ln_DataFrame | None = None,
			datalogger: DataLogger | None = None,
			persist_path: PATH_TYPE | None = None, name=None,
			**kwargs, ) -> None:
	...
	def add_message(
			...
			output_fields=None, recipient=None, **kwargs, ) -> None:
		"""
		...
		_msg = MessageUtil.create_message(
			...
			response=response, output_fields=output_fields,
...
# sourcery skip: merge-nested-ifs
	...
	def action_request(self) -> dataframe.ln_DataFrame:
		...
		return convert.to_df(
			self.messages[self.messages.sender == "action_request"]
			)
	...
	def action_response(self) -> dataframe.ln_DataFrame:
		...
		return convert.to_df(
			self.messages[self.messages.sender == "action_response"]
			)
	...
	def assistant_responses(self) -> dataframe.ln_DataFrame:
		...
		a_responses = self.responses[
			self.responses.sender != "action_response"]
	...
	def describe(self) -> dict[str, Any]:
		...
		return {
			...
			"messages": [msg.to_dict() for _, msg in
			             self.messages.iterrows()][
			            : len(self.messages) - 1 if len(
				            self.messages
				            ) < 5 else 5],
		}
	...
	@classmethod
	def _from_csv(
			cls, filename: str, read_kwargs=None, **kwargs
			) -> "BaseBranch":
	...
	def to_csv_file(
			self, filename: PATH_TYPE = "messages.csv",
			dir_exist_ok: bool = True, timestamp: bool = True,
			time_prefix: bool = False, verbose: bool = True,
		...
		filename = SysUtil.create_path(
			self.datalogger.persist_path, filename, timestamp=timestamp,
			dir_exist_ok=dir_exist_ok, time_prefix=time_prefix, )
	...
	def to_json_file(
			self, filename: PATH_TYPE = "messages.json",
			dir_exist_ok: bool = True, timestamp: bool = True,
			time_prefix: bool = False, verbose: bool = True,
			clear: bool = True, **kwargs, ) -> None:
		"""
	...
	def log_to_csv(
			self, filename: PATH_TYPE = "log.csv", dir_exist_ok: bool = True,
			timestamp: bool = True, time_prefix: bool = False,
			verbose: bool = True, clear: bool = True, flatten_=True,
			sep="[^_^]", **kwargs, ) -> None:
		"""
		...
		self.datalogger.to_csv_file(
			filename=filename, dir_exist_ok=dir_exist_ok, timestamp=timestamp,
			time_prefix=time_prefix, verbose=verbose, clear=clear,
			flatten_=flatten_, sep=sep, **kwargs, )

	def log_to_json(
			self, filename: PATH_TYPE = "log.json", dir_exist_ok: bool = True,
			timestamp: bool = True, time_prefix: bool = False,
			verbose: bool = True, clear: bool = True, flatten_=True,
			sep="[^_^]", **kwargs, ) -> None:
		"""
	...
	def load_log(
			self, filename, flattened=True, sep="[^_^]", verbose=True,
			**kwargs
			):
		df = ""
		try:
			...
			for _, row in df.iterrows():
				self.datalogger.log.append(
					DLog.deserialize(
						input_str=row.input_data, output_str=row.output_data,
						unflatten_=flattened, sep=sep, )
				)
	...
	def change_first_system_message(
			self, system: str | dict[str, Any] | System,
	...
	def replace_keyword(
			self, keyword: str, replacement: str, column: str = "content",
			case_sensitive: bool = False, ) -> None:

		dataframe.replace_keyword(
			self.messages, keyword, replacement, column=column,
			case_sensitive=case_sensitive, )

	def search_keywords(
			self, keywords: str | list[str], case_sensitive: bool = False,
			reset_index: bool = False,
			dropna: bool = False, ) -> dataframe.ln_DataFrame:
		return dataframe.search_keywords(
			self.messages, keywords, case_sensitive=case_sensitive,
			reset_index=reset_index, dropna=dropna, )
	...
	def filter_by(
			self, role: str | None = None, sender: str | None = None,
			start_time=None, end_time=None,
			content_keywords: str | list[str] | None = None,
			case_sensitive: bool = False, ) -> dataframe.ln_DataFrame:

		return MessageUtil.filter_messages_by(
			self.messages, role=role, sender=sender, start_time=start_time,
			end_time=end_time, content_keywords=content_keywords,
			case_sensitive=case_sensitive, )
	...
	def _info(self, use_sender: bool = False) -> dict[str, int]:
		...
		messages = self.messages["sender"] if use_sender else self.messages[
			"role"]
