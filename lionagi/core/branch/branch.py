...
class Branch(BaseBranch, BranchFlowMixin):
	...
	def __init__(
			...
			service: BaseService | None = None, sender: str | None = None,
			...
			tool_manager: ToolManager | None = None, **kwargs, ):

		# init base class
		super().__init__(
			messages=messages, datalogger=datalogger,
			persist_path=persist_path, name=name, **kwargs, )
	...
	@classmethod
	def from_csv(
			cls, filepath, name: str | None = None,
			service: BaseService | None = None,
			llmconfig: dict[str, str | int | dict] | None = None,
			tools: TOOL_TYPE | None = None,
			datalogger: None | DataLogger = None,
			persist_path: PATH_TYPE | None = None, # instruction_sets=None,
			tool_manager: ToolManager | None = None, read_kwargs=None,
			**kwargs, ):
	...
	def messages_describe(self) -> dict[str, Any]:
		...
		return dict(
			total_messages=len(self.messages), summary_by_role=self._info(),
	...
	def delete_tools(
			self, tools: Union[T, list[T], str, list[str]],
		...
		if convert.is_same_dtype(tools, str):
			...
			for act_ in tools:
				if act_.schema_["function"][
					"name"] in self.tool_manager.registry:
	...
	def receive(
			self, sender: str, messages: bool = True, tools: bool = True,
		...
		while self.pending_ins[sender]:
			...
			if mail_.category == "messages" and messages:
				...
				self.messages = self.messages.merge(
					mail_.package, how="outer"
					)
