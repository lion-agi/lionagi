# exact copy from lion_core.setting
...
def format_deprecated_msg(
    ...
    msg = f"<{deprecated_type}>: <{deprecated_name}> is deprecated since <{deprecated_version}>."
    if removal_version:
        msg += f" It will be removed in {removal_version}."
    if replacement is None:
        msg += " No replacement is available."
    elif replacement == "python":
        msg += f" Use the Python directly: <{additional_msg}> instead."
...
class SchemaModel(BaseModel):
    ...
    schema_version: int | float | str | None = Field(default=None, exclude=True)
