...
class QueryNode:
    def __init__(self, id: int, query: str, tool: Optional[str] = None, dependencies: List[int] = []):
        self.id = id
        self.query = query
        self.tool = tool
...
class QueryPlanTool:
    def __init__(self, tools: Dict[str, Any], response_synthesizer: Any, name: str, description_prefix: str):
    ...
    def get_metadata(self):
        description = f"{self.description_prefix}\n\n" + "\n\n".join(
            [f"Tool Name: {name}\nTool Description: {tool}" for name, tool in self.tools_dict.items()]
        )
        return {"description": description, "name": self.name}

    def execute_node(self, node: QueryNode, nodes_dict: Dict[int, QueryNode]):
        if node.dependencies:
            child_responses = [self.execute_node(nodes_dict[dep], nodes_dict) for dep in node.dependencies]
            combined_response = self.response_synthesizer.synthesize(node.query, child_responses)
            if node.tool:
