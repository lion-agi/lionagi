...
class Refine:
    ...
    def __init__(self, llm, prompt_helper, text_qa_template, refine_template, output_cls=None):
        self.llm = llm
    ...
    async def get_response(
        self, query_str: str, text_chunks: Sequence[str], prev_response: Optional[str] = None, **response_kwargs: Any
    ) -> str:
        response = None
        for text_chunk in text_chunks:
            if prev_response is None:
                response = await self._agive_response_single(query_str, text_chunk, **response_kwargs)
            else:
                response = await self._arefine_response_single(prev_response, query_str, text_chunk, **response_kwargs)
    ...
    async def _give_response_single(
        ...
        for cur_text_chunk in text_chunks:
            response = await self.llm.apredict(text_qa_template, context_str=cur_text_chunk, **response_kwargs)
        return response
    ...
    async def _refine_response_single(
        ...
        refine_template = self.refine_template.format(query_str=query_str, existing_answer=response)
        text_chunks = self.prompt_helper.repack(refine_template, text_chunks=[text_chunk])

        for cur_text_chunk in text_chunks:
            structured_response = await self.llm.apredict(refine_template, context_msg=cur_text_chunk, **response_kwargs)
