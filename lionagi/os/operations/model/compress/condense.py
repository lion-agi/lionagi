...
class SimpleChatEngine(BaseChatEngine):
    ...
    def __init__(self, llm: LLM, memory: BaseMemory, prefix_messages: List[ChatMessage]):
        self._llm = llm
        self._memory = memory
        self._prefix_messages = prefix_messages

    @classmethod
    def from_defaults(cls, chat_history: Optional[List[ChatMessage]] = None, memory: Optional[BaseMemory] = None, memory_cls: Type[BaseMemory] = ChatMemoryBuffer, system_prompt: Optional[str] = None, prefix_messages: Optional[List[ChatMessage]] = None, llm: Optional[LLM] = None, **kwargs):
        llm = llm or LLM.from_defaults()
        chat_history = chat_history or []
        memory = memory or memory_cls.from_defaults(chat_history=chat_history, llm=llm)
        if system_prompt:
            prefix_messages = [ChatMessage(content=system_prompt, role=llm.metadata.system_role)]
    ...
    def stream_chat(self, message: str, chat_history: Optional[List[ChatMessage]] = None) -> StreamingAgentChatResponse:
        ...
        self._memory.put(ChatMessage(content=message, role="user"))
        all_messages = self._prefix_messages + self._memory.get()
        chat_response = StreamingAgentChatResponse(chat_stream=self._llm.stream_chat(all_messages))
    ...
    async def astream_chat(self, message: str, chat_history: Optional[List[ChatMessage]] = None) -> StreamingAgentChatResponse:
        ...
        self._memory.put(ChatMessage(content=message, role="user"))
        all_messages = self._prefix_messages + self._memory.get()
        chat_response = StreamingAgentChatResponse(achat_stream=await self._llm.astream_chat(all_messages))
        return chat_response
...
class ContextChatEngine(BaseChatEngine):
    ...
    def __init__(self, retriever: BaseRetriever, llm: LLM, memory: BaseMemory, prefix_messages: List[ChatMessage], node_postprocessors: Optional[List[BaseNodePostprocessor]] = None, context_template: Optional[str] = None):
    ...
    def _generate_context(self, message: str) -> Tuple[str, List[NodeWithScore]]:
        ...
        context_str = "\n\n".join([n.node.get_content(MetadataMode.LLM).strip() for n in nodes])
    ...
    def chat(self, message: str, chat_history: Optional[List[ChatMessage]] = None) -> AgentChatResponse:
        ...
        prefix_messages = [ChatMessage(content=context_str, role="system")] + self._prefix_messages
        all_messages = prefix_messages + self._memory.get()
        chat_response = self._llm.chat(all_messages)
        ai_message = chat_response.message
        self._memory.put(ai_message)
        return AgentChatResponse(response=str(chat_response.message.content), sources=[ToolOutput(tool_name="retriever", content=context_str)], source_nodes=nodes)
...
class CondenseQuestionChatEngine(BaseChatEngine):
    ...
    def __init__(self, query_engine: BaseQueryEngine, condense_question_prompt: PromptTemplate, memory: BaseMemory, llm: LLM, verbose: bool = False):
    ...
    def _condense_question(self, chat_history: List[ChatMessage], last_message: str) -> str:
        ...
        return self._llm.predict(self._condense_question_prompt, question=last_message, chat_history=chat_history_str)

    async def _acondense_question(self, chat_history: List[ChatMessage], last_message: str) -> str:
        if not chat_history:
            return last_message
        chat_history_str = "\n".join([str(m) for m in chat_history])
        return await self._llm.apredict(self._condense_question_prompt, question=last_message, chat_history=chat_history_str)
    ...
    def chat(self, message: str, chat_history: Optional[List[ChatMessage]] = None) -> AgentChatResponse:
        ...
        return AgentChatResponse(response=str(query_response), sources=[ToolOutput(content=str(query_response), tool_name="query_engine")])
