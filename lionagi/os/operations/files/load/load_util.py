...
def dir_to_path(
    dir: str, ext: str | list, recursive: bool = False, flatten: bool = True
) -> List[Path]:
    """
    ...
    try:
        ext = ext if isinstance(dir, list) else [ext]
        dir = [_dir_to_path(i) for i in ext]
        return to_list(dir, flatten=flatten)
...
def dir_to_nodes(
    ...
    """
     directory contents into Node objects based on specified file extensions.

    This function first retrieves a list of file paths from the specified directory, matching the given file extension. It then reads the content of these files, optionally cleaning the text, and  each file's content into a Node object.
    ...
    Example:
        ...
        # ng them into Node objects.
    """

    path_list = dir_to_path(dir=dir_, ext=ext, recursive=recursive, flatten=flatten)
    files_info = [read_text(file, clean=clean_text) for file in path_list]
    return [Node(content=i[0], metadata=i[1]) for i in files_info]
...
def file_to_chunks(
    ...
    **kwargs,
    ...
    return to_list(
        [chunk_func(i, **kwargs) for i in to_list(input)]
        )
