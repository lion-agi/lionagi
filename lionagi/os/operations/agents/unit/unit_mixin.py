...
from lionagi.os.collections.message.util import _parse_action_request
...
class DirectiveMixin(ABC):
    ...
    async def _extend(
        ...
        **kwargs,
        ...
        if plan:
            ...
            plan = force_validate_mapping(plan, keys)
    ...
    async def _act(self, form, branch, actions=None):
        ...
        actions = force_validate_mapping(actions, keys)
    ...
    def _process_model_response(content_, requested_fields):
        ...
        if requested_fields:
            with contextlib.suppress(Exception):
                return force_validate_mapping(out_, requested_fields)
