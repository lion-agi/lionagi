...
# ChatCompletion
oai_chat_llmconfig = {
	"model": "gpt-4-turbo-preview", "frequency_penalty": 0,
	"max_tokens": None, "n": 1, "presence_penalty": 0,
	"response_format": {"type": "text"}, "seed": None, "stop": None,
	"stream": False, "temperature": 0.7, "top_p": 1, "tools": None,
	"tool_choice": "none", "user": None,
}
...
oai_chat_schema = {
	...
	"optional": ["seed", "stop", "stream", "tools", "tool_choice", "user",
		"max_tokens", ], "input_": "messages", "config": oai_chat_llmconfig,
}

# Finetune
oai_finetune_llmconfig = {
	"model": "gpt-3.5-turbo", "hyperparameters": {
		"batch_size": "auto", "learning_rate_multiplier": "auto",
		"n_epochs": "auto",
	}, "suffix": None, "training_file": None,
}

oai_finetune_schema = {
	"required": ["model", "training_file"],
	"optional": ["hyperparameters", "suffix", "validate_file"],
	"input_": ["training_file"], "config": oai_finetune_llmconfig,
}

# Audio ---- create  speech

oai_audio_speech_llmconfig = {
	"model": "tts-1", "voice": "alloy", "response_format": "mp3", "speed": 1,
}
oai_audio_speech_schema = {
	"required": ["model", "voice"], "optional": ["response_format", "speed"],
	"input_": "input_", "config": oai_audio_speech_llmconfig,
}

# Audio ----------- create transcription
oai_audio_transcriptions_llmconfig = {
	"model": "whisper-1", "language": None, "prompt": None,
	"response_format": "json", "temperature": 0,
}
oai_audio_transcriptions_schema = {
	"required": ["model", "voice"],
	"optional": ["response_format", "language", "prompt", "response_format",
		"temperature", ], "input_": "file",
...
oai_audio_translations_schema = {
	...
	"input_": "file", "config": oai_audio_translations_llmconfig,
}
...
oai_schema = {
	"chat/completions": oai_chat_schema, "finetune": oai_finetune_schema,
