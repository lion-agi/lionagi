...
class BooleanRule(Rule):
    ...
    async def validate(self, value, annotation=None):
        if self.condition(annotation):
            try:
                return validation_funcs["bool"](value, fix_=self.fix, **self.validation_kwargs)
            except Exception as e:
                raise ValueError(f"failed to validate field") from e
...
class NumberRule(Rule):
    ...
    def condition(self, annotation=None):
        return any([i in annotation for i in ["int", "float", "number"]]) and "str" not in annotation
    ...
    async def validate(self, value, annotation=None):
        if self.condition(annotation):
            ...
            try:
                return validation_funcs["number"](value, fix_=self.fix, **self.validation_kwargs)
            except Exception as e:
                raise ValueError(f"failed to validate field") from e
    ...
    async def validate(self, value, annotation=None, keys=None):
        if self.condition(annotation):
            if "str" not in annotation or keys:
                try:
                    return validation_funcs["dict"](value, keys=keys, fix_=self.fix, **self.validation_kwargs)
                except Exception as e:
                    raise ValueError(f"failed to validate field") from e
            raise ValueError(f"failed to validate field") from e
...
class StringRule(Rule):
    ...
    async def validate(self, value, annotation=None):
        if self.condition(annotation):
            try:
                return validation_funcs["str"](value, fix_=self.fix, **self.validation_kwargs)
            except Exception as e:
                raise ValueError(f"failed to validate field") from e
