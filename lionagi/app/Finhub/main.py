...
class FinnHub:
    """
    A utility class for interacting with the FinnHub API to retrieve financial
    and market data.

    Attributes:
        api_key (str): The API key used for authenticating with the FinnHub
            API.
    ...
    Methods:
        ...
        get_info_df(info_kind="company_news", api_key=None, **kwargs):
            Retrieves data from the FinnHub API based on the specified
            'info_kind' and returns it as a Pandas DataFrame.
    """
    ...
    def get_client(cls, api_key=None):
        ...
        Args:
            api_key (str): The API key used for authenticating with the FinnHub
                API. If not provided, the class-level API key (if set) will be
                used.
        ...
        Raises:
            ImportError: If there is an error while importing the 'finnhub'
                library.
        """
        try:
            from lionagi.os.libs.sys_util import check_import

            check_import("finnhub")
    ...
    def get_info_df(cls, info_kind="company_news", api_key=None, **kwargs):
        """
        Retrieves data from the FinnHub API based on the specified 'info_kind'
        and returns it as a Pandas DataFrame.

        Args:
            info_kind (str): The type of information to retrieve from FinnHub.
                Default is "company_news".
            api_key (str): The API key used for authenticating with the FinnHub
                API. If not provided, the class-level API key (if set) will be
                used.
        ...
        Raises:
            ValueError: If there is an error during the API call or while
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Error occurred during converting {info_kind} to "
                    f"DataFrame: {e}"
                )
        except Exception as e:
            raise ValueError(
    ...
    def get_finnhub_method(client, info_kind):
        """
        Returns the appropriate FinnHub API method based on the specified
        'info_kind'.
        ...
        Returns:
            Callable or None: The corresponding FinnHub API method, or None if
                'info_kind' is not recognized.
        """
        info_func = {
