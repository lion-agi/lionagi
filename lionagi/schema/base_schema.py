...
from ..utils import get_timestamp, create_path, to_dict, create_id, change_dict_key, is_schema, to_df
import json
...
# ------ conversion methods ------
    ...
    def pd_series(self):
        """
        Converts the node to a pandas Series object.

        Returns:
            A pandas Series object representing the node.

        Examples:
            >>> node = BaseNode(content="Example content")
            >>> series = node.to_pd_series()
        """
        return Series(self.dict)
