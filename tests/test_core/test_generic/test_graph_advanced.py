...
from lionagi.core.typing import RelationError
...
class TestGraphPerformance:
    ...
    def test_bulk_operations(self):
        ...
        for node in nodes_to_remove:
            if node.ln_id in graph.internal_nodes:
...
class TestGraphConcurrency:
    ...
    async def test_concurrent_node_additions(self):
        ...
        assert len(graph.internal_nodes) == num_operations
        assert all(node.ln_id in graph.internal_nodes for node in nodes)
    ...
    async def test_concurrent_edge_additions(self):
        ...
        assert all(edge.ln_id in graph.internal_edges for edge in edges)
    ...
    async def test_concurrent_mixed_operations(self):
        ...
        for node1, node2, edge in results:
            assert node1.ln_id in graph.internal_nodes
            assert node2.ln_id in graph.internal_nodes
            assert edge.ln_id in graph.internal_edges
            assert edge.ln_id in graph.node_edge_mapping[node1.ln_id]["out"]
            assert edge.ln_id in graph.node_edge_mapping[node2.ln_id]["in"]
...
class TestGraphAdvancedOperations:
    ...
    def test_graph_with_large_properties(self):
        ...
        assert (
            len(
                graph.internal_edges[edge.ln_id].properties.get(
                    "large_property"
                )
            )
            == 1000000
    ...
    def test_graph_stress(self):
        ...
        for _ in range(num_nodes * operations_per_node):
            ...
            if operation == "add_edge":
                ...
                tail = random.choice(nodes)
                if (
                    head.ln_id in graph.internal_nodes
                    and tail.ln_id in graph.internal_nodes
                ):
                    try:
                        edge = Edge(head=head, tail=tail)
                        graph.add_edge(edge)
        ...
        for edge in graph.internal_edges.values():
            ...
            assert edge.ln_id in graph.node_edge_mapping[edge.head]["out"]
            assert edge.ln_id in graph.node_edge_mapping[edge.tail]["in"]
