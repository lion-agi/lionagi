...
from lionagi.core.generic.edge import Edge, EdgeCondition
from lionagi.core.generic.graph import Graph
from lionagi.core.generic.node import Node
from lionagi.core.typing import Note, RelationError
...
class TestGraphBasics:
    ...
    def test_empty_graph_creation(self, empty_graph):
        ...
        assert isinstance(empty_graph.node_edge_mapping, Note)

    def test_add_node(self, empty_graph):
        """Test adding a node"""
        node = TestNode()
        empty_graph.add_node(node)
        assert node.ln_id in empty_graph.internal_nodes
        assert empty_graph.node_edge_mapping[node.ln_id] == {
            "in": {},
            "out": {},
        }
    ...
    def test_add_edge(self, simple_graph):
        ...
        assert edge.ln_id in graph.internal_edges
        assert (
            graph.node_edge_mapping[node1.ln_id]["out"][edge.ln_id]
            == node2.ln_id
        )
        assert (
            graph.node_edge_mapping[node2.ln_id]["in"][edge.ln_id]
            == node1.ln_id
        )
...
class TestGraphTraversal:
    ...
    def test_get_predecessors(self, complex_graph):
        ...
        pred_ids = {node.ln_id for node in predecessors}
        assert nodes[0].ln_id in pred_ids
        assert nodes[2].ln_id in pred_ids
    ...
    def test_get_successors(self, complex_graph):
        ...
        succ_ids = {node.ln_id for node in successors}
        assert nodes[1].ln_id in succ_ids
        assert nodes[3].ln_id in succ_ids
...
class TestGraphModification:
    ...
    def test_remove_node(self, simple_graph):
        ...
        assert node1.ln_id not in graph.internal_nodes
        assert edge.ln_id not in graph.internal_edges
        assert node1.ln_id not in graph.node_edge_mapping
    ...
    def test_remove_edge(self, simple_graph):
        ...
        assert edge.ln_id not in graph.internal_edges
        assert edge.ln_id not in graph.node_edge_mapping[node1.ln_id]["out"]
        assert edge.ln_id not in graph.node_edge_mapping[node2.ln_id]["in"]
