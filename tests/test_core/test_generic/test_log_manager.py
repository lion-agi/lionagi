...
def sample_log():
    ...
    return Log(
        content=Note(message="test message"), loginfo=Note(level="INFO")
    )
...
def test_log_manager_capacity(sample_log):
    ...
    new_log = Log(
        content=Note(message="second message"), loginfo=Note(level="INFO")
    )
    manager.log(new_log)
...
async def test_log_manager_concurrent_logging():
    ...
    async def add_log(i):
        log = Log(
            content=Note(message=f"message {i}"), loginfo=Note(level="INFO")
        )
