...
class TestGraphTraversal:
    ...
    def test_get_predecessors(self, traversal_graph):
        ...
        assert nodes[1].ln_id in pred_ids
    ...
    def test_get_successors(self, traversal_graph):
        ...
        assert nodes[4].ln_id in succ_ids
        assert nodes[5].ln_id in succ_ids
    ...
    def test_find_node_edge_in(self, traversal_graph):
        ...
        assert len(edges) == 2
        for edge in edges:
            assert edge.tail == nodes[3].ln_id
    ...
    def test_find_node_edge_out(self, traversal_graph):
        ...
        assert len(edges) == 2
        for edge in edges:
            assert edge.head == nodes[3].ln_id
...
class TestGraphTraversalEdgeCases:
    ...
    def test_disconnected_components_traversal(self):
        ...
        heads = graph.get_heads()
        assert len(heads) == 2
        head_ids = {node.ln_id for node in heads}
        assert node1.ln_id in head_ids
        assert node3.ln_id in head_ids
    ...
    def test_multi_edge_traversal(self):
        ...
        assert all(
            edge.head == node1.ln_id and edge.tail == node2.ln_id
            for edge in edges
        )
